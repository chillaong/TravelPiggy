...
- **WebP 优先并自动回退**：前端运行时检测浏览器对 WebP 的支持，优先加载同名 `.webp` 资源（若已在项目中生成），若加载失败或浏览器不支持则回退到原始 JPG/PNG，避免显示空白或报错。
- **延迟加载非首屏资源**：地图与相册的背景图、商店/背包的大图等，在进入相应场景或渲染时才设置 `background-image` 或赋 `src`，图片使用 `loading="lazy"` 的场景也已增加，减少首屏请求数量。
- **首屏关键资源预加载**：保留对主背景与关键小图（如主角图标）的优先加载配置，以提高首屏可交互速度。
- **离线压缩建议**：请在本地构建阶段将 `assets` 下的大图批量压缩为 WebP/AVIF（例如使用 `cwebp -q 80`），再将原图保留为回退，通常能节省 30–60% 的带宽。

示例批量转换命令（macOS + Homebrew）：
